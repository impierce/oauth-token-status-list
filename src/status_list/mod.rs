...
impl StatusList {
    ...
    pub fn get_status(&self, index: usize) -> Result<u8, OAuthTSLError> {
    ...
    pub fn set_status(&mut self, index: usize, value: u8) -> Result<(), OAuthTSLError> {
        ...
        match index_input {
            ...
            IndexInput::Multiple(values) => {
                ...
                for (i, &index) in indices.iter().enumerate() {
                    self.set_status(index, values[i])?;
                }
            }
        }
        Ok(())
...
mod test {
    ...
    pub fn test_compress_example_1() {
        ...
        status_list.set_status(0, 1).unwrap();
        status_list.set_status(1993, 1).unwrap();
        status_list.set_status(25460, 1).unwrap();
        status_list.set_status(159495, 1).unwrap();
        status_list.set_status(495669, 1).unwrap();
        status_list.set_status(554353, 1).unwrap();
        status_list.set_status(645645, 1).unwrap();
        status_list.set_status(723232, 1).unwrap();
        status_list.set_status(854545, 1).unwrap();
        status_list.set_status(934534, 1).unwrap();
        status_list.set_status(1000345, 1).unwrap();

        assert_eq!(status_list.get_status(1000345).unwrap(), 1);
    ...
    pub fn test_compress_example_2() {
        ...
        // I did not use fn set_index_array here to mimic the example from the spec as much as possible.
        status_list.set_status(0, 1).unwrap();
        status_list.set_status(1993, 2).unwrap();
        status_list.set_status(25460, 1).unwrap();
        status_list.set_status(159495, 3).unwrap();
        status_list.set_status(495669, 1).unwrap();
        status_list.set_status(554353, 1).unwrap();
        status_list.set_status(645645, 2).unwrap();
        status_list.set_status(723232, 1).unwrap();
        status_list.set_status(854545, 1).unwrap();
        status_list.set_status(934534, 2).unwrap();
        status_list.set_status(1000345, 3).unwrap();

        assert_eq!(status_list.get_status(1000345).unwrap(), 3);
    ...
    pub fn test_compress_example_3() {
        ...
        status_list.set_status(0, 1).unwrap();
        status_list.set_status(1993, 2).unwrap();
        status_list.set_status(35460, 3).unwrap();
        status_list.set_status(459495, 4).unwrap();
        status_list.set_status(595669, 5).unwrap();
        status_list.set_status(754353, 6).unwrap();
        status_list.set_status(845645, 7).unwrap();
        status_list.set_status(923232, 8).unwrap();
        status_list.set_status(924445, 9).unwrap();
        status_list.set_status(934534, 10).unwrap();
        status_list.set_status(1000345, 12).unwrap();
        status_list.set_status(1004534, 11).unwrap();
        status_list.set_status(1030203, 13).unwrap();
        status_list.set_status(1030204, 14).unwrap();
        status_list.set_status(1030205, 15).unwrap();

        assert_eq!(status_list.get_status(1030205).unwrap(), 15);
    ...
    pub fn test_compress_example_4() {
        ...
        status_list
            .set_status_array(indices, IndexInput::Multiple(values))
            .unwrap();

        assert_eq!(status_list.get_status(19535).unwrap(), 255);
